...
fn no_adapter_error() -> String {
    if cfg!(windows) {
        "Bluetooth adapter not found. Please enable Bluetooth in Windows Settings.".into()
    } else if cfg!(target_os = "android") {
        "Bluetooth adapter not found. Please enable Bluetooth and grant Bluetooth/Location permissions."
            .into()
    } else if cfg!(target_os = "macos") {
        "Bluetooth adapter not found. Please enable Bluetooth in System Settings.".into()
    } else {
        "Bluetooth adapter not found.".into()
    }
}
