...
pub struct TerminalSessionCore {
    ...
    pub handle: rsterm_connection::ConnectionHandle,
    pub terminal: Terminal,
    pub active_port: u8,
    ...
    pub metrics: rsterm_remote::SessionMetrics,
    pub session_sftp: Option<std::sync::Arc<rsterm_fs::sftp::SftpClient>>,
...
impl ActiveSession {
    ...
    pub fn paste_text(&mut self, text: &str) -> crate::ConnectionViewAction {
        ...
        let mut action = crate::ConnectionViewAction::None;
        let _ = crate::drain_connection(self, &mut action);
        action
    }
    ...
    pub fn tab_label(&self) -> String {
        match self.core.conn_type {
            ...
            ConnectionType::Local | ConnectionType::Ssh => {
                if let Some(cmd) =
                    rsterm_platform::get().foreground_command(self.core.handle.shell_pid)
                {
                    return rsterm_platform::get().truncate_label(&cmd, 32);
                }
                let title = self.core.terminal.screen.title.trim();
                if !title.is_empty()
                    && !rsterm_platform::get().title_is_idle_host(title, &self.core.user_at_host)
                {
                    return rsterm_platform::get().truncate_label(title, 32);
                }
