...
//!
//! Each [`crate::terminal::ActiveSession`] owns a cache. List fetches are driven by
...
use crate::terminal::ActiveSession;
    ...
    pub fn handle_inbound_drop(
        ...
        match conn_type {
            ConnectionType::Local => {
                for src in paths {
                    let dest = rsterm_platform::dnd::dest_path(Path::new(&cwd), src);
                    if src.is_dir() {
...
fn tick_local(session: &mut ActiveSession) {
    ...
    if let Some(ref cwd) = cwd {
        ...
        let home = rsterm_fs::home_dir().to_string_lossy().into_owned();
...
fn ensure_session_sftp(
    ...
    let auth = connect_params::ssh_auth(conn, auth_user);
