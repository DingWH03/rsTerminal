...
use rsterm_connection::{ConnIn, ConnectionState};
use rsterm_terminal::parser::TermEvent;
...
pub fn drain_connection(session: &mut ActiveSession, action: &mut ConnectionViewAction) -> bool {
    ...
    for ev in session.core.metrics.drain_events() {
        let line = rsterm_remote::format_metrics_event(&ev);
        match &ev {
            rsterm_remote::MetricsEvent::Status(_) => {
...
mod tests {
    use super::*;
    use rsterm_connection::ConnectionPort;
