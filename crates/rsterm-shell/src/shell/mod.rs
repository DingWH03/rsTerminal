...
use crate::layout::{DropZone, PaneId};
...
use crate::shell::layout_state::{FUNCTION_MAX_WIDTH, FUNCTION_MIN_WIDTH};
...
impl AppShell {
    ...
    fn session_label(sessions: &[WorkspaceSession], id: &str) -> String {
        sessions
            ...
            .map(|s| {
                format!(
                    "{} {}",
                    crate::connection_display::workspace_session_icon(s),
                    s.tab_label()
                )
            })
    ...
    fn pane_label(sessions: &[WorkspaceSession], layout: &ShellLayout, pane_id: PaneId) -> String {
        layout
            ...
            .unwrap_or_else(|| crate::i18n_bridge::tr("empty_pane"))
    }
    ...
    pub fn render(
        ...
        favorite_commands: &[rsterm_data::persist::types::FavoriteCommand],
        auth_users: &[rsterm_data::persist::types::AuthUser],
        ...
        if show_workspace {
            ...
            if let Some(drag) = &self.active_drag {
                let palette_len = crate::pane_colors::resolve_palette(prefs).len().max(1);
        ...
        if self.layout.ui.connections_dialog_open {
            let frame = DialogFrame::new(crate::i18n_bridge::tr("dialog_open_connections"));
        ...
        if self.layout.ui.help_dialog_open {
            let frame = DialogFrame::alert(crate::i18n_bridge::tr("menu_about"));
            let mut dismiss = false;
            if frame.show(ui.ctx(), "about_dialog", |ui| {
                ui.label(
                    egui::RichText::new("rsTerminal")
                        .size(crate::uiframe::tokens::text::HEADING)
                        .strong(),
                );
                ui.add_space(crate::uiframe::tokens::space::MD);
                ui.label(format!(
                    "{} {}",
                    crate::i18n_bridge::tr("about_version"),
                    env!("CARGO_PKG_VERSION")
                ));
                ui.add_space(crate::uiframe::tokens::space::LG);
                ui.label(crate::i18n_bridge::tr("about_placeholder"));
                ui.add_space(crate::uiframe::tokens::space::XL);
                let ok_label = crate::i18n_bridge::tr("ok");
                let close_btn = crate::uiframe::style::primary_button(&ok_label)
                    .min_size(egui::vec2(80.0, crate::uiframe::tokens::size::BUTTON));
