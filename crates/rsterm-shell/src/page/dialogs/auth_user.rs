...
impl AuthUserDialog {
    ...
    pub fn show(&mut self, ctx: &egui::Context) -> Option<AuthUser> {
        ...
        let title = if self.edit_id.is_some() {
            crate::i18n_bridge::tr("auth_user_edit_title")
        } else {
            crate::i18n_bridge::tr("auth_user_new_title")
        };
        ...
        let closed = frame.show(ctx, "auth_user_dialog", |ui| {
            ...
            form::labeled_row(ui, crate::i18n_bridge::tr("auth_user_method"), |ui| {
                ui.radio_value(
                    ...
                    AuthMethod::Password,
                    crate::i18n_bridge::tr("auth_user_method_password"),
                );
                ui.radio_value(
                    &mut self.auth_method,
                    AuthMethod::PrivateKey,
                    crate::i18n_bridge::tr("auth_user_method_key"),
                );
            });
            ...
            match self.auth_method {
                AuthMethod::Password => {
                    form::labeled_password(
                        ui,
                        crate::i18n_bridge::tr("auth_user_password"),
                        &mut self.password,
                    );
                }
                AuthMethod::PrivateKey => {
                    form::labeled_row(ui, crate::i18n_bridge::tr("auth_user_private_key"), |ui| {
                        #[cfg(not(target_os = "android"))]
                        {
                            if ui
                                .button(crate::i18n_bridge::tr("auth_user_pick_key_file"))
                                .clicked()
                                && let Some(path) = rfd::FileDialog::new()
                                    .set_title(crate::i18n_bridge::tr("auth_user_pick_key_file"))
                                    .pick_file()
...
pub fn auth_users_page(
    ...
    form::section_card(ui, |ui| {
        if form::manage_list_toolbar(ui, crate::i18n_bridge::tr("auth_users_manage_new")) {
            action.new = true;
        }
        ...
        if auth_users.is_empty() {
            ...
            ui.label(
                egui::RichText::new(crate::i18n_bridge::tr("settings_users_empty"))
        ...
        for user in auth_users {
            let method = match user.auth_method {
                AuthMethod::Password => crate::i18n_bridge::tr("auth_user_method_password"),
                AuthMethod::PrivateKey => crate::i18n_bridge::tr("auth_user_method_key"),
            };
...
pub fn manage_auth_users_dialog(
    ...
    let frame = DialogFrame::new(crate::i18n_bridge::tr("auth_users_manage_title"));
