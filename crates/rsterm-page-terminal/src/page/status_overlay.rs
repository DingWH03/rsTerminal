use rsterm_connection::ConnectionState;
...
pub(super) fn render(
    ...
    if let Some(msg) = session.core.disconnect_message.as_ref() {
        return Some(render_disconnected(ui, session, area_size, msg, &labels));
    }
    if matches!(session.core.handle.state, ConnectionState::Connecting) {
        ...
        egui::Frame::new().fill(scrim).show(ui, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.label(
                    egui::RichText::new(&labels.connecting)
...
fn render_disconnected(
    ...
    egui::Frame::new().fill(scrim).show(ui, |ui| {
        ...
        ui.vertical_centered(|ui| {
            ...
            if can_reconnect {
                let btn = style::primary_button(&labels.reconnect)
