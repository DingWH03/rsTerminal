...
use crate::ui::function_pane::common::nav_button;
...
pub fn paint_home_sidebar(
    ...
    if nav_button(
        ui,
        Some(crate::ui::uiframe::vector_icons::Icon::Settings),
    ...
    let empty_visible = std::collections::HashSet::new();
    let ctx = crate::ui::function_pane::session_list::SessionListContext {
        split_enabled: false,
        visible_sessions: &empty_visible,
        session_accents: &empty_accents,
    };
    let sessions_action = paint_session_rows(ui, sessions, active_id, &ctx);

    HomeSidebarResult {
        nav: nav_action,
        sessions: SessionRowAction {
            select_session: sessions_action.select_session,
            close_session: sessions_action.close_session,
            new_window_session: sessions_action.duplicate_session,
        },
    }
}
