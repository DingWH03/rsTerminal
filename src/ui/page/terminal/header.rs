...
pub(super) fn render(
    ...
    let mut center = |ui: &mut egui::Ui| {
        if show_actions {
            ui.scope(|ui| {
                ...
                if text_toolbar_button(ui, ui.id().with("sel_copy"), rust_i18n::t!("copy"))
                    .clicked()
                {
                    copy = true;
                }
                if text_toolbar_button(ui, ui.id().with("sel_paste"), rust_i18n::t!("paste"))
                    .clicked()
                {
                    paste = true;
                }
                if text_toolbar_button(ui, ui.id().with("sel_cancel"), rust_i18n::t!("cancel"))
                    .clicked()
                {
                    cancel_sel = true;
                }
    ...
    if toggle_kb {
        ...
        if keyboard.visible {
            ...
            crate::ui::page::terminal::input::hide_android_terminal_ime(ui.ctx());
        }
    }
}
